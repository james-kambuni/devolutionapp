...
function SettingsLayout() {
  ...
  const filteredRoutes = settingsRoute.filter(route => 
    ...
    <div className={`flex h-screen bg-gray-50 dark:bg-gray-900 ${isSidebarOpen && 'overflow-hidden'}`}>
      ...
      <div className="flex flex-col flex-1 w-full overflow-hidden">
        ...
        <div className="flex flex-col md:flex-row relative overflow-hidden">
          ...
          {isSettingsSidebarOpen && (
            <div className="fixed inset-0 z-20 bg-black opacity-50 lg:hidden" onClick={toggleSettingsSidebar}></div>
          )}
              ...
              <ul className="space-y-2 font-medium">
                {filteredRoutes.map((route) =>
                  route.routes ? (
                    ...
                    <li className="relative px-6 py-3 text-gray-800 dark:text-gray-200" onClick={()=>{
                      setIsSettingsSidebarOpen(false)
                    }} key={route.name}>
                      <NavLink
                        exact
