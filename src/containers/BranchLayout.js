...
function BranchLayout() {
  ...
  const toggleBranchSidebar = () => {
    setIsBranchSidebarOpen(!isBranchSidebarOpen);
  };
  ...
  const filteredRoutes = branchRoute.filter(route => 
    ...
    <div className={`flex h-screen bg-gray-50 dark:bg-gray-900 ${isSidebarOpen && 'overflow-hidden'}`}>
      ...
      <div className="flex flex-col flex-1 w-full overflow-hidden">
        ...
        <div className="flex flex-col md:flex-row relative">
          <button
            onClick={toggleBranchSidebar}
            type="button"
            className="inline-flex items-center p-2 mt-2 ms-3 text-sm text-gray-500 rounded-lg sm:hidden hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-gray-200 dark:text-gray-400 dark:hover:bg-gray-700 dark:focus:ring-gray-600"
          >
            <span className="sr-only">Open branch sidebar</span>
            <svg
              className="w-6 h-6"
              aria-hidden="true"
              fill="currentColor"
          ...
          {isBranchSidebarOpen && (
            <div className="fixed inset-0 z-20 bg-black opacity-50 lg:hidden" onClick={toggleBranchSidebar}></div>
          )}
          ...
          <div
            ...
            className={`fixed pb-20 pt-5 inset-y-0 left-0 z-40 w-64 h-screen overflow-auto scrollbar-thin scrollbar-thumb-gray-500 scrollbar-track-gray-300 transition-transform duration-300 transform bg-gray-100 dark:bg-gray-800 lg:static lg:inset-0 ${
              isBranchSidebarOpen ? 'translate-x-0' : '-translate-x-full'
            } lg:translate-x-0`}
          >
