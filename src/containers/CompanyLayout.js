...
function CompanyLayout() {
  ...
  const toggleCompanySidebar = () => {
    setIsCompanySidebarOpen(!isCompanySidebarOpen);
  };

  const handleClickOutside = (event) => {
    if (sidebarRef.current && !sidebarRef.current.contains(event.target)) {
      setIsCompanySidebarOpen(false);
    }
  };

  useEffect(() => {
    if (isCompanySidebarOpen) {
  ...
  const filteredRoutes = companyRoute.filter(route => 
    ...
    <div className={`flex h-screen bg-gray-50 dark:bg-gray-900 ${isSidebarOpen && 'overflow-hidden'}`}>
      ...
      <div className="flex flex-col flex-1 w-full overflow-hidden">
        ...
        <div className="flex flex-col md:flex-row relative overflow-hidden">
          ...
          >
            <span className="sr-only">Open company sidebar</span>
            <svg
              className="w-6 h-6"
              aria-hidden="true"
              fill="currentColor"
          ...
          {isCompanySidebarOpen && (
            <div className="fixed inset-0 z-20 bg-black opacity-50 lg:hidden" onClick={toggleCompanySidebar}></div>
          )}
          ...
          <div
            ...
            className={`fixed pb-20 pt-5 inset-y-0 left-0 z-40 w-64 h-screen overflow-auto scrollbar-thin scrollbar-thumb-gray-500 scrollbar-track-gray-300 transition-transform duration-300 transform bg-gray-100 dark:bg-gray-800 lg:static lg:inset-0 ${
              isCompanySidebarOpen ? 'translate-x-0' : '-translate-x-full'
            } lg:translate-x-0`}
          >
